...
def get_chat(chat_id, pageToken, f_today, yt_api):
  # setting parameters
...
def main(yt_url, yt_api):
  ...
  while(chat_id):
    try:
      nextPageToken = get_chat(chat_id, nextPageToken, f_today, yt_api)
      time.sleep(slp_time)
    except:
      break
  ...
  while(chat_id):
    try:
      nextPageToken = get_chat(chat_id, nextPageToken, f_today, YT_API_KEY_BACK_UP)
      time.sleep(slp_time)
